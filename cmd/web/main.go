...
func main() {
	...
	r.Handle("/checkout?id={id:[0-9]+}", s.traceHandler(logHandler(s.checkout))).Methods(http.MethodGet)
	r.Handle("/addproduct?id={id:[0-9]+}", s.traceHandler(logHandler(s.addProduct))).Methods(http.MethodGet)
	srv := http.Server{
